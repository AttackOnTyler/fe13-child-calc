// PROTOTYPE — host page = the chosen pairing-table layout (variant D, copied from ../pairing-table)
// with hooks so each skill-tab variant can mount itself: an extra per-row cell, an inline row
// expansion, a replacement centre view, or a replacement right panel.
...
function childTable(s: State, all: Row[], pass: (r: Row) => boolean) {
  ...
  const colHead = `<tr>${th(s, 'parent', 'Variable parent', 'stick k0')}${H.rowCell ? '<th>Skills</th>' : ''}<th>Robin</th>
  ...
  const body = shownRows
    .map((r) => {
      ...
      const main = `<tr data-row="${r.key}" class="${r.cls == null ? 'dead' : ''} ${multi ? 'grouprow' : ''} ${H.selectedKey === r.key ? 'selrow' : ''}" ${multi ? `data-ctl="expand" data-v="${r.groupKey}"` : ''}>
