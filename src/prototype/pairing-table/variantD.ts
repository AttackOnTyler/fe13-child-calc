// PROTOTYPE — Variant D "B + A + C": B's child rail + always-on scoring panel; the child view uses
// A's per-stat column groups with toggles; Robin's 56 combos collapse to one row whose heatmap picks
// which combo the row shows. An "All children" rail entry shows C's leaderboard across everyone.
import { AF_STATS, CHILDREN, CHILD_NAMES, STATS, sortRows, type Row, type State } from './data';
import * as W from './controls';
import { board } from './variantC';

export const name = 'By child + columns + heatmap (+ All = leaderboard)';
...
function childTable(s: State, all: Row[], pass: (r: Row) => boolean) {
  ...
  const shownRows = sortRows([...groups.keys()], s);

  const groupHead = `<tr class="grp"><th colspan="2" class="stick k0">Pairing</th><th colspan="${g.speed ? 3 : 2}">Result</th>
  ...
  const colHead = `<tr>${th(s, 'parent', 'Variable parent', 'stick k0')}<th>Robin</th>
  ...
  const ncols = 5 - (g.speed ? 0 : 1) + (g.caps ? 8 : 0) + (g.mods ? 7 : 0) + (g.growths ? 8 : 0) + (g.build ? 1 : 0) + 1;
  ...
  const body = shownRows
    .map((r) => {
      ...
      const main = `<tr class="${r.cls == null ? 'dead' : ''} ${multi ? 'grouprow' : ''}" ${multi ? `data-ctl="expand" data-v="${r.groupKey}"` : ''}>
        <td class="stick k0">${multi ? (open ? '▾' : '▸') : ''} ${r.parentLabel} ${W.flags(r)}</td>
      ...
      return multi && open ? main + `<tr class="expand"><td colspan="${ncols}">${heatmap(gr, s, isMorgan)}</td></tr>` : main;
    })
    .join('');
  ...
  return {
    ...
    html: `<table class="grid vd-table"><thead>${groupHead}${colHead}</thead><tbody>${body}</tbody></table>`,
  };
}

export function render(root: HTMLElement, s: State, rows: Row[], total: number, all: Row[]) {
  ...
  const rail = ['All', ...CHILD_NAMES]
    .map((c) => `<button class="rail-item ${c === s.selectedChild ? 'on' : ''} ${c === 'All' ? 'all' : ''}" data-ctl="selectChild" data-v="${c}">
      <span>${c === 'All' ? 'All children' : c}</span><b>${c === 'All' ? (bestAll < 0 ? '—' : bestAll) : bestBy.get(c) ?? '—'}</b></button>`)
    .join('');
  ...
  root.innerHTML = `<div class="vb vd">
    ...
    <main class="vb-main">
      ...
      <div class="vb-scroll">${view.html}</div>
    </main>
